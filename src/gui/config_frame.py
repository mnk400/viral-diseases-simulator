...
class SetConfigFrame(tk.Frame):
    ...
    def create_widgets(self):
        ...
        population_scale = tk.Scale(master=label_frame, variable=tk.IntVar(), orient='horizontal', from_=1000, to=100000, width=25, sliderlength=100, length=float(label_frame.winfo_reqwidth())*0.75)
        ...
        # label2 = tk.Label(master=label_frame, anchor='center', text='Social Distancing Ratio', pady=15)
        # label2.pack(expand=True, fill='both')
        # label2.grid_propagate(0)
        # label2.grid(row=1, column=0, padx=20)
        # social_distance_scale = tk.Scale(master=label_frame, variable=tk.DoubleVar(), orient='horizontal', from_=0, to= 1)
        # social_distance_scale.pack(expand=True, fill='both')
        # social_distance_scale.grid(row=1, column=1, padx=20)
        # social_distance_scale.grid_propagate(0)
        #
        # label3 = tk.Label(master=label_frame, anchor='center', text='Hospital capacity', pady=15)
        # label3.pack(expand=True, fill='both')
        # label3.grid_propagate(0)
        # label3.grid(row=2, column=0, padx=20)
        # hospital_capacity_scale = tk.Scale(master=label_frame, variable=tk.DoubleVar(), sliderlength=150, orient='horizontal', from_=1000, to= 5000)
        # hospital_capacity_scale.pack(expand=True, fill='both')
        # hospital_capacity_scale.grid(row=2, column=1, padx=20)
        # hospital_capacity_scale.grid_propagate(0)
        #
        # label4 = tk.Label(master=label_frame, anchor='center', text='Recovery time', pady=15)
        # label4.pack()
        # recovery_time_scale = tk.Scale(master=label_frame, variable=tk.IntVar(), sliderlength=150, orient='horizontal', from_=1, to=50)
        # recovery_time_scale.pack(expand=True, fill='both')
        #
        # label5 = tk.Label(master=label_frame, anchor='center', text='Death rate', pady=15)
        # label5.pack()
        # death_rate_scale = tk.Scale(master=label_frame, variable=tk.IntVar(), sliderlength=150, orient='horizontal', from_=0, to=100)
        # death_rate_scale.pack(expand=True, fill='both')
        #
        # val_frame = tk.Frame(master=label_frame, width=label_frame.winfo_width())
        ...
        # label6.pack_propagate(0)
        #
        # r_val = tk.StringVar()
        # r_field = tk.Entry(master=val_frame, textvariable=r_val, width=5)
        # r_val_button = tk.Button(master=val_frame, text='Set r value', command='')
        # r_field.pack(side='right')
        # r_field.grid(row=0, column=1, padx=20)
