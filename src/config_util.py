...
class ConfigUtil(object):
    ...
    def __init__(self, path_param: str):
        """
        Constructor responsible for calling the load function to load
        configuration data from the file. This constructor loads the default
        specified path as the config file if no other file path is input.
    ...
    def getBooleanValue(self, section_str: str, key_str: str) -> bool:
        ...
        if self.configLoaded is True:
            ...
            try:
                ...
                # return false and log the event if the key could not be
                # retrieved
                logging.error(e)
                return None
        else:
    ...
    def hasConfigData(self) -> bool:
        ...
        """
        if self.configLoaded is not False:
            ...
            for section in self.sections:
                ...
                if len(key_set) > 1 or list(key_set)[0] != 'Not set':
                    ...
                    # return a false if no unique keys are found and the file
                    # is empty
                    return False
        else:
            # return false if the file has not been loaded
    ...
    def loadConfigData(self) -> bool:
        ...
        if os.path.exists(self.filepath):
            # Loading the file into the parser if it exists then logging and
            # then returning a true
